function [ Output ] = Load_MNEsensor_to_ft_wrapper(subs, runs, source, source_cond, event_cond, event_type, sr, prestim, poststim, RLflag)
...
%function will load sensor data, not source estimates!
%
%   Inputs:
...
%       source_cond - sensor file's prefix. Source files are usually like
...
for sub = 1:size(subs,2)
    ...
    for run = runs
        ...
        fname = fullfile(source_dir,[num2str(subs(sub)) '_' source_cond '_run' num2str(run) '_sss_ds_raw.fif']);
        if RLflag ==0
        ...
        data{run} = MEG_load_label_source_trial(fname, ename, sr, prestim, poststim);
    end
    cfg=[];
