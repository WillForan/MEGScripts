...
%MEG data will be writen to a new fiff file. Note it is assumed input has
...
ecg_component = find(abs(corr(data.trial{1}(309,:)',comp.trial{1}(1:64,:)'))>0.5);
if ~ any(ecg_component)&& ekg_flag ==1
    ifg.runica.pca = 36;
    comp = ft_componentanalysis(ifg,data);
    ecg_component = find(abs(corr(data.trial{1}(309,:)',comp.trial{1}(1:36,:)'))>0.1);

    veog_component = find(abs(corr(data.trial{1}(307,:)',comp.trial{1}(1:36,:)'))>0.5);
    if any(veog_component)
        fprintf('\n\n\n')
        disp(['Removing ' num2str(size(veog_component,2)) ' eye blink components ' ]);
    end
    heog_component = find(abs(corr(data.trial{1}(308,:)',comp.trial{1}(1:36,:)'))>0.5);
    ...
    veog_component = find(abs(corr(data.trial{1}(307,:)',comp.trial{1}(1:64,:)'))>0.5);
    if any(veog_component)
        fprintf('\n\n\n')
        disp(['Removing ' num2str(size(veog_component,2)) ' eye blink components ' ]);
    end
    heog_component = find(abs(corr(data.trial{1}(308,:)',comp.trial{1}(1:64,:)'))>0.5);
    if any(heog_component)
        fprintf('\n\n\n')
        disp(['Removing ' num2str(size(heog_component,2)) ' saccade components ' ]);
    end
...
ecg_component = find(abs(corr(data.trial{1}(309,:)',comp.trial{1}(1:64,:)'))>0.5);
if ~ any(ecg_component)&& ekg_flag ==999 % EKGs are ususally not visable in gradiometers
    ifg.runica.pca = 36;
    comp = ft_componentanalysis(ifg,data);
    ecg_component = find(abs(corr(data.trial{1}(309,:)',comp.trial{1}(1:36,:)'))>0.5);
