function [ CorrectTrials InCorrectTrials AllTrials ] = MEG_write_response_locked_events_RT( InputData, stim, trig, delay, write_flag )
...
for run = runs
    ...
    if write_flag == 1
        fname=strcat(stim,'-RL-run',num2str(run),'-All-RT.eve');
        dlmwrite(fname, AllTrials{run}, 'delimiter', '\t',  'precision', 10);
        
        fname=strcat(stim,'-RL-run',num2str(run),'-Correct-RT.eve');
        dlmwrite(fname, CorrectTrials{run}, 'delimiter', '\t',  'precision', 10);
        
        fname=strcat(stim,'-RL-run',num2str(run),'-Incorrect-RT.eve');
