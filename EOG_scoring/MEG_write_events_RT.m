function [ CorrectTrials InCorrectTrials AllTrials ] = MEG_write_events_RT( InputData, stim, trig, delay, write_flag )
% This function will take outputs from ScoreMEGSaccades.m and create event
% files for MNE pipeline. This function will loop through runs in the mat
% file and create event files separately for each run. This version will
% also append the RT to the end.
%
...
%
%   Written by Kai Hwang, last update 10.07.2011
...
for run = runs
    ...
    evetFirstVector = [double(data{run}.initial_offset), double(data{run}.initial_offset)*double(1/data{run}.sr), 0, 0, 0];
    ...
    for t=1:1:size(data{run}.Scored_saccadeTable,1)
        if any(data{run}.Scored_saccadeTable(t,3) == trig) && data{run}.Scored_saccadeTable(t,8) == 1
            CeventVector = [CeventVector; ...
                ...
                double(data{run}.initial_offset + data{run}.Scored_saccadeTable(t,2)    ...
                + round(delay/(1000/data{run}.sr))) * double(1/data{run}.sr),   ...
                0, trig(1), data{run}.Scored_saccadeTable(t,4)];
        end 
    end
    ...
    for t=1:1:size(data{run}.Scored_saccadeTable,1)
        if any(data{run}.Scored_saccadeTable(t,3) == trig) && data{run}.Scored_saccadeTable(t,8) == 1
            eventVector = [eventVector; ...
                ...
                double(data{run}.initial_offset + data{run}.Scored_saccadeTable(t,2)    ...
                + round(delay/(1000/data{run}.sr))) * double(1/data{run}.sr),   ...
                0, 1, data{run}.Scored_saccadeTable(t,4)];
        elseif any(data{run}.Scored_saccadeTable(t,3) == trig) && data{run}.Scored_saccadeTable(t,8) == 0
            eventVector = [eventVector; ...
                ...
                double(data{run}.initial_offset + data{run}.Scored_saccadeTable(t,2)    ...
                + round(delay/(1000/data{run}.sr))) * double(1/data{run}.sr),   ...
                0, 2, data{run}.Scored_saccadeTable(t,4)];
        end 
    end
    ...
    if write_flag == 1
        fname=strcat(stim,'-run',num2str(run),'-All-RT.eve');
        dlmwrite(fname, AllTrials{run}, 'delimiter', '\t',  'precision', 10);
        
        fname=strcat(stim,'-run',num2str(run),'-Correct-RT.eve');
        dlmwrite(fname, CorrectTrials{run}, 'delimiter', '\t',  'precision', 10);
        
        fname=strcat(stim,'-run',num2str(run),'-Incorrect-RT.eve');
