function [ CorrectTrials InCorrectTrials AllTrials ] = MEG_write_events( InputData, stim, trig, delay, write_flag )
...
%           subjn_anit. The program will then append run number
%           and .eve after it.
%
...
%           is important for accounting for accurate timing. The last time
%           I measured wit the block anti/vgs paradigm the delay is 51ms.
%
...
for run = runs
    ...
    if write_flag == 1
        fname=strcat(stim,'-run',num2str(run),'-All.eve');
        dlmwrite(fname, AllTrials{run}, 'delimiter', '\t',  'precision', 10);
        
        fname=strcat(stim,'-run',num2str(run),'-Correct.eve');
        dlmwrite(fname, CorrectTrials{run}, 'delimiter', '\t',  'precision', 10);
        
        fname=strcat(stim,'-run',num2str(run),'-Incorrect.eve');
